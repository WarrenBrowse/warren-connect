...
impl VerifiedRequest {
    ...
    /// effect, so each signed request is acted on at most once. A request the
    /// gate refuses spends nothing, and the wallets no gate admits, which
    /// cost nothing to mint, never reach the store. A
...
/// Proof that a route admitted a wallet-signed request: its gate let it
/// through and its nonce is spent. Only [`VerifiedRequest::admit`] makes one,
/// and the store calls a signed route acts through (a login approval, a forum
/// link write, a parked or delivered report, a notification read or write)
/// each take one, so a route that forgets its admission does not compile;
/// a forgotten call would leave that route replayable and open to wallets
/// its gate was meant to refuse. It names no wallet: each call still says
/// which forum identity it acts for.
#[derive(Debug)]
#[must_use]
pub struct Admitted {
    _private: (),
...
mod tests {
    ...
    fn a_nonce_is_remembered_for_as_long_as_its_request_can_be_admitted() {
        ...
        let _nonce_spent = verify_signed_request(&h, "POST", "/p", b"", earliest)
    ...
    fn a_request_is_admitted_up_to_two_minutes_after_its_window_and_not_after() {
        ...
        let _nonce_spent = verify_signed_request(&on_time, "POST", "/p", b"", 1_060)
    ...
    fn rejects_a_replayed_nonce() {
        ...
        let _nonce_spent = verify_signed_request(&h, "POST", "/p", b"", 1_000)
