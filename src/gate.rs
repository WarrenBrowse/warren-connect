...
pub struct Gates {
    ...
    /// which costs a DiscourseConnect nonce from the forum and ends at the
    /// first "never paid", so it is the costly read to flood and it has a
    /// bulkhead of its own that no flood of the open routes can fill.
